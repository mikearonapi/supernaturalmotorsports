 ...
 * Uses explicit perf scores if available, otherwise derives them from advisory scores and hard metrics
 * @param {Object} car - Car object
 * @returns {Object} - Performance scores by category key
 */
export function getStockPerformanceScores(car) {
  // If the car has explicit Performance HUB scores (not null/undefined), use them
  // These are manually curated scores that take precedence over derived values
  if (car.perfPowerAccel != null && car.perfPowerAccel !== undefined) {
    return {
  ...
  // Otherwise, derive from existing scores using our algorithm
  // The algorithm uses hard metrics (0-60, braking, lateral G) when available
 ...
 * @returns {Object} - Cost range { low, high, display, tier, tierLabel }
 */
export function calculateTotalCost(selectedUpgrades, car = null) {
  ...
  for (const upgrade of selectedUpgrades) {
    if (car) {
      ...
      totalLow += adjusted.low;
      totalHigh += adjusted.high;
    } else {
