 ...
 */
const BLOB_BASE_URL = process.env.NEXT_PUBLIC_VERCEL_BLOB_URL || 'https://abqnp7qrs0nhv5pw.public.blob.vercel-storage.com';
...
export function getCarHeroImage(car) {
  ...
  // 2. Try local images in public folder (primary for this deployment)
  if (car.slug) {
    return LOCAL_CAR_IMAGE_PATHS.hero(car.slug);
  }
  
  // 3. Try Vercel Blob using slug (fallback)
  if (car.slug && BLOB_BASE_URL) {
    return getBlobUrl(CAR_IMAGE_PATHS.hero(car.slug));
  }
  
  // 4. No image available - return null for placeholder
  return null;
}
...
export function getCarThumbnail(car) {
  ...
  // Try local thumbnail (same as hero for now)
  if (car.slug) {
    return LOCAL_CAR_IMAGE_PATHS.thumbnail(car.slug);
  }
  
  // Fall back to hero image - we use the same image for thumbnails
  // since hero images work well at smaller sizes
...
export function getPageImage(key) {
  // 1. Try local images first (primary for this deployment)
  const localPath = LOCAL_PAGE_IMAGE_PATHS[key];
  if (localPath) {
    return localPath;
  }
...
export function hasCarImages(car) {
  return !!(
    ...
    car.slug || // Local images are available for all cars with slugs
